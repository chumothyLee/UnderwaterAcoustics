...
function defaultShouldUpdateScroll(prevRouterProps, nextRouterProps) {
  if (_gatsbyBrowser.shouldUpdateScroll) {
    return (0, _gatsbyBrowser.shouldUpdateScroll)(prevRouterProps, nextRouterProps);
  }
  if (prevRouterProps) {
    var oldPathname = prevRouterProps.location.pathname;
    var pathname = nextRouterProps.location.pathname;
...
loadConfig(function () {
  return loadContext(function (pagesReq) {
    ...
    if (_gatsbyBrowser.modifyRoutes) {
      routes = (0, _gatsbyBrowser.modifyRoutes)(routes);
    }
    ...
    var Root = function Root() {
      return _react2.default.createElement(_reactRouter.Router, {
        ...
        routes: routes,
        render: (0, _reactRouter.applyRouterMiddleware)((0, _useScroll2.default)(defaultShouldUpdateScroll)),
        onUpdate: onUpdate
      });
    };

    if (_gatsbyBrowser.wrapRootComponent) {
      Root = (0, _gatsbyBrowser.wrapRootComponent)(Root);
    }

    if (_gatsbyBrowser.replaceDOMRenderer) {
      (0, _gatsbyBrowser.replaceDOMRenderer)({ routes: routes, defaultShouldUpdateScroll: defaultShouldUpdateScroll, onUpdate: onUpdate });
    } else {
      _reactDom2.default.render(_react2.default.createElement(Root, null), typeof window !== 'undefined' ? document.getElementById('react-mount') : undefined);
    }
  });
});